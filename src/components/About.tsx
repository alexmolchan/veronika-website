...
const icons: Record<string, JSX.Element> = {
  shield: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 text-grass"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
  heart: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 text-grass"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>,
  clock: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 text-grass"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>,
  lock: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 text-grass"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>,
}
...
export default function About({ content }: { content: any }) {
  ...
  return (
    <section id="about" className="py-28 px-[5%] bg-off-white">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.3fr] gap-20 items-center max-w-6xl mx-auto">
        <div className="max-w-lg mx-auto lg:max-w-none">
          {content?.aboutImage ? (
            <img src={urlFor(content.aboutImage).width(500).height(550).url()} alt="Обо мне" className="w-full h-[400px] lg:h-[550px] object-cover" />
          ) : (
            <img src="/images/about.jpg" alt="Обо мне" className="w-full h-[400px] lg:h-[550px] object-cover" />
          )}
        </div>
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-10 pt-10 border-t border-sand">
            {features.map((f, i) => (
